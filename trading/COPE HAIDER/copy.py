...
RISK_PERCENT = 0.05        # 5% مخاطرة
FALLBACK_LOT = 0.01        # اللوت الثابت إذا كان الـ SL كبير جداً
...
def get_cleaned_symbol(raw_symbol):
    # 1. إذا كانت العملة حالة خاصة (مثل GOLD) نأخذها من القاموس
    if raw_symbol in SYMBOL_MAPPING:
        return SYMBOL_MAPPING[raw_symbol]
    
    # 2. إذا كان حساب المستثمر فيه زوائد مثل EURUSD.micro نحذف النقطة وما بعدها
    clean_name = raw_symbol
    if "." in raw_symbol:
        clean_name = raw_symbol.split('.')[0]
        
    # 3. إضافة علامة التعجب (!) لكل العملات أوتوماتيكياً لحسابك!
    return clean_name + "!"
...
def calculate_lot_size(symbol, entry_price, sl_price, balance):
    ...
    # حماية: إذا كان الـ SL سيخسرك أكثر من 5%، أجبر اللوت على 0.01
    if calculated_lot < FALLBACK_LOT:
        print(f"⚠️ SL for {symbol} is > 5% of balance! Forcing lot to {FALLBACK_LOT}")
        return FALLBACK_LOT

    if calculated_lot < min_lot: return min_lot
...
def sync_all_data(inv_pos_dict, current_time):
    ...
    for inv_id, inv_pos in inv_pos_dict.items():
        if inv_id not in pos_map:
            ...
            # فلتر: إذا لم يجد السعر سيعطيك تنبيه بدلاً من الصمت
            if not tick: 
            ...
            if res and res.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                # إضافة وهمية لمنع التكرار اللحظي
                class Dummy: pass
                d = Dummy()
...
try:
    while True:
        ...
        # 3. طباعة رسالة الانتظار
        print(f"[{current_time}] 💤 Waiting 5 seconds...\n")
        print("-" * 40) # خط فاصل لترتيب الشاشة
